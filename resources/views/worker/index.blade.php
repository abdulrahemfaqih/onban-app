...
@section('content')
    {{-- Worker Profile --}}
    <div class="flex">
        <img class="w-[100px] h-[100px] rounded-full border-4 border-primary" src="{{ asset('storage/' . $worker->foto_formal) }}" alt="">
        <div class="pl-3">
    ...
    @if ($status_menerima_order)
        <div class="w-full max-w-md p-4 bg-white border border-gray-400 rounded-lg shadow">
            ...
            <div class="flow-root h-40 overflow-y-auto">
                @if ($orders_menunggu->isEmpty() && $orders_proses->isEmpty())
                    <div class="flex items-center justify-center h-full">
                        <p class="text-gray-500">Belum Ada Pesanan</p>
                    </div>
                @else
                    <ul role="list" class="divide-y divide-gray-300">
                        @foreach ($orders_menunggu as $order)
                            <li class="py-3">
                                    ...
                                    <div class="items-center text-base font-semibold text-gray-900 text-center">
                                        ...
                                        <div class="pt-1">
                                            <a href="{{ route('worker-order-detail', ['id_order' => $order->id_order]) }}">
                                                <button class="text-xs bg-primary text-white hover:bg-primary-dark rounded-lg p-2">Lihat Detail</button>
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @endforeach
                        @foreach ($orders_proses as $order)
                            <li class="py-3">
                                <div class="flex items-center">
                                    <div class="flex-1 min-w-0 ">
                                        <p class="text-sm font-sm text-gray-900 truncate">
                                    ...
                                    <div class="items-center text-base font-semibold text-gray-900 text-center">
                                        @if ($order->status_order == 'Diproses')
                                            ...
                                            <a href="{{ route('worker-order', ['id_order' => $order->id_order]) }}">
                                                <button class="text-xs bg-primary text-white hover:bg-primary-dark rounded-lg p-2">Lanjutkan Orderan</button>
                                            </a>
